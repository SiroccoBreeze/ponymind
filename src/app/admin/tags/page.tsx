...
export default function TagsManagement() {
  ...
  const handleDeleteTag = async (tagId: string, tagName: string) => {
    ...
    setUpdating(tagId);
    try {
      const response = await fetch(`/api/admin/tags?tagId=${tagId}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">添加标签</h2>
      ...
      {showEditModal && selectedTag && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">编辑标签</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">标签名称 *</label>
                <input
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {predefinedColors.map((color) => (
                    <button
                      key={color}
                      onClick={() => setSelectedTag({ ...selectedTag, color })}
                      className={`w-8 h-8 rounded-full border-2 ${
                        selectedTag.color === color ? 'border-gray-800' : 'border-gray-300'
                      }`}
                      style={{ backgroundColor: color }}
                    />
                  ))}
